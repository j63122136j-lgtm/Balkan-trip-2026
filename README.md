...
- 09/23 Split 藍洞、Vis、Hvar 五島快艇 Klook 已付款，兩人實付 NT$8,714。
- 集合點改為共和廣場，06:10 離開住宿、06:40 前報到、07:00 出發，全程約 11 小時。
- 補入訂單 KJQ575284、09/22 00:00 前取消期限，並同步門票與旅行總額。
- 快取版本提升至 v4.24.0。

## v4.23.0 更新

- 09/18 改為已付款的 Klook Postojna Cave＋Predjama Castle 半日團，兩人共 NT$8,354。
- 補入 08:00 InterContinental Ljubljana 集合、07:45 報到、訂單編號與取消期限。
- 09/19 Bled／Vintgar 改為 Bled Central 接駁、09:30 完整 5.7 km 路線，並加入賽事人潮提醒。
- 預算與行前票券清單同步更新，快取版本提升至 v4.23.0。

## v4.22.0 更新

- 頂部導覽列移除 ONLINE／OFFLINE 狀態，改為所選 Day 主要城市的即時當地時間。
- 當地時間會依 TPE、BKK、IST、LJU、ZAG、SPU、OMO、SJJ、DBV、GVA、PVG 時區自動換算，每 30 秒更新。
- 頂欄移除「加入主畫面」按鈕；More 頁仍保留離線功能說明與次要安裝入口。

## v4.21.0 更新

- 09/14 去程改為已預約機場接送：17:30 從「竹南三隻貓媽媽」出發，直達桃園機場第一航廈。
- 兩人接送費 NT$1,685 納入固定支出；移除 Day 1 原本的高鐵／機捷替代規劃。
- 固定／預計支出由 NT$123,495 更新為 NT$125,180，包含機票、接送與 16 晚住宿。

## v4.20.0 更新

- Day 選單第二行由日期改為當天主要落腳城市，手機可更快辨識行程位置。
- 當日往返行程沿用住宿城市；跨城日顯示目的地城市。
- 日期仍保留在每日內容與無障礙標籤中，不會遺失日期資訊。
