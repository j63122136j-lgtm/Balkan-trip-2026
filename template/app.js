(() => {
  ...
  function renderDay(){
    ...
    const food=d.food?.length?`<section class="discovery-section discovery-food"><span class="kicker">EAT HERE</span><h2>順路餐廳</h2><div class="food-list">${d.food.map(x=>`${x.image?`<figure class="food-photo"><img src="${x.image.src}" alt="${x.image.alt}" loading="lazy" referrerpolicy="no-referrer" onerror="this.closest('figure').remove()"><figcaption><a href="${x.image.source}" target="_blank" rel="noopener">${x.image.credit}</a></figcaption></figure>`:''}<a href="${maps(x.map)}" target="_blank" rel="noopener"><div class="food-heading"><b>${esc(x.name)}</b>${x.pick?`<em class="food-pick">${esc(x.pick)}</em>`:''}</div><span>${esc(x.dish)}</span>${x.note?`<p>${esc(x.note)}</p>`:''}${x.price||x.booking?`<small class="food-meta">${[x.price,x.booking].filter(Boolean).map(esc).join(' · ')}</small>`:''}<i>Maps ↗</i></a>`).join('')}</div></section>`:'';
