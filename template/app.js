(() => {
  ...
  function renderDay(){
    ...
    $('#day-detail').innerHTML=`<article class="day-hero reveal"><div class="day-hero-top"><div><span class="kicker">${esc(d.country)} · ${esc(d.theme)}</span><h2>${esc(d.city)}</h2><p>${esc(d.summary)}</p></div><div class="day-hero-tools"><span class="day-weather-pill" id="day-weather-pill"><b>◌</b><span>${esc(city.label)}</span><strong>--°</strong></span>${lodging}</div></div>${heroPhoto}</article>${itinerary}${dayExpenseCard(d)}<div class="day-info-grid compact-info">${food}${local}</div>`;
    bindExpenseCard(d);
